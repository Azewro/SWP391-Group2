...
public class AdminUserPromotionDAO {
    ...
    // Lấy danh sách user_id của khách hàng chưa từng đặt vé
    public List<Integer> getNewUsers() {
        List<Integer> newUsers = new ArrayList<>();
        String query = "SELECT user_id FROM Users WHERE user_id NOT IN (SELECT DISTINCT user_id FROM Orders)";

        try (PreparedStatement stmt = conn.prepareStatement(query)) {
            ResultSet rs = stmt.executeQuery();

            while (rs.next()) {
                newUsers.add(rs.getInt("user_id"));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
